...
`asm/syscalls32.asm` provides CDECL-callable wrappers for:
...
It documents `EAX` as the syscall-number/return register, syscall argument registers, and the fact that raw kernel failures are **negative errno values**, not necessarily `-1`.

A no-libc assembly demo is available through:
...
`make asm` is optional and requires NASM plus 32-bit ELF linker support.
...
The server intentionally has no non-loopback binding option and no persistence or authentication-bypass behavior. It is a local systems-programming demonstration, not a remote-administration product.
...
Requirements for the main toolkit:

- Linux or WSL2
- GCC or compatible C compiler
- GNU Make
- pthreads / standard Linux development headers
...
- quoted arguments
- arbitrary pipeline structure
- redirection syntax and invalid pipeline errors
- allocator allocation/free behavior
- allocator concurrent stress
- ELF validation and memory-map parsing
- entry-point virtual-address translation
- mapping a purpose-built freestanding static ELF
- shell pipeline smoke execution
- loopback TCP client/server request-response behavior

The code has also been exercised with AddressSanitizer + UndefinedBehaviorSanitizer during development.
...
Key concepts exercised across modules:
...
- The parser intentionally implements a useful shell subset rather than the full POSIX shell grammar.
- The custom loader rejects dynamically linked `PT_INTERP` executables.
- Entry-point execution is intended for specially built freestanding static executables; general Linux process startup also requires stack/aux-vector/interpreter setup normally performed by the kernel.
- ELF32 execution requires a 32-bit build environment, although ELF32 inspection/mapping logic is present in the portable build.
- `ptrace attach` is subject to the host kernel's Yama/ptrace permissions.
